...
const ListarAvaliacoesFornecedorModal = ({ isOpen, onClose, fornecedorId, fornecedorNome }: Props) => {
    ...
    return (
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-4">
            ...
            <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-2xl animate-in zoom-in-95 duration-200 overflow-hidden flex flex-col max-h-[85vh]">
                <div className="bg-slate-900 p-6 flex justify-between items-start shrink-0">
                    ...
                    <button onClick={onClose} className="p-1.5 text-slate-400 hover:bg-white/10 hover:text-white transition-colors rounded-lg flex-shrink-0">
                        <X size={20} />
                    </button>
                </div>
                ...
                <div className="p-6 overflow-y-auto custom-scrollbar flex-1 bg-slate-50/50">
                    {loading ? (
                        ...
                        <div className="space-y-4">
                            {avaliacoes.map((aval) => (
                                <div key={aval.id} className="bg-white border border-slate-200 rounded-xl p-5 shadow-sm">
                                    <div className="flex justify-between items-start mb-3">
                                        <div>
                                            <div className="font-bold text-slate-800">{aval.utilizador.username}</div>
                                    ...
                                    {aval.comentario && (
                                        <div className="mt-3 bg-slate-50 rounded-lg p-3 text-sm text-slate-700 border border-slate-100 italic">
                                            "{aval.comentario}"
                                        </div>
                                    )}

                                    <div className="mt-4 grid grid-cols-5 gap-2 pt-4 border-t border-slate-100">
                                        <div className="text-center">
                                            <div className="text-[10px] font-bold text-slate-400 uppercase">Qualid.</div>
                                            <div className="text-sm font-semibold text-slate-700">{aval.qualidade}/5</div>
                                        </div>
                                        <div className="text-center">
                                            <div className="text-[10px] font-bold text-slate-400 uppercase">Pontual.</div>
                                            <div className="text-sm font-semibold text-slate-700">{aval.pontualidade}/5</div>
                                        </div>
                                        <div className="text-center">
                                            <div className="text-[10px] font-bold text-slate-400 uppercase">Comun.</div>
                                            <div className="text-sm font-semibold text-slate-700">{aval.comunicacao}/5</div>
                                        </div>
                                        <div className="text-center">
                                            <div className="text-[10px] font-bold text-slate-400 uppercase">Preço</div>
                                            <div className="text-sm font-semibold text-slate-700">{aval.preco}/5</div>
                                        </div>
                                        <div className="text-center">
                                            <div className="text-[10px] font-bold text-slate-400 uppercase">Confor.</div>
                                            <div className="text-sm font-semibold text-slate-700">{aval.conformidade}/5</div>
                                        </div>
                                    </div>
                                </div>
                            ))}
                        ...
                        <div className="flex flex-col items-center justify-center h-48 text-center text-slate-500">
                            ...
                            <p className="font-semibold text-slate-700">Sem avaliações para este fornecedor.</p>
                        </div>
                    )}
                </div>
            </div>
