            ...
            <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-xl w-full max-w-4xl max-h-[90vh] flex flex-col overflow-hidden animate-in zoom-in-95 duration-200">
                ...
                <div className="flex items-center justify-between px-6 py-5 border-b border-slate-100 dark:border-slate-700/50 bg-slate-50 dark:bg-slate-900/80">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <h2 className="text-xl font-bold text-slate-900 dark:text-slate-100 leading-tight">Histórico de Entradas de Stock</h2>
                            <p className="text-xs text-slate-500 dark:text-slate-400 font-medium mt-0.5">Produtos recebidos via receção de encomendas</p>
                        </div>
                    </div>
                    <button
                        onClick={onClose}
                ...
                {/* Search + Stats */}
                <div className="px-6 py-4 border-b border-slate-100 dark:border-slate-700/50 bg-white dark:bg-slate-800 space-y-3">
                    <div className="relative">
                        ...
                        <input
                            ...
                            className="w-full pl-9 pr-4 py-2.5 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl text-sm outline-none focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-400 transition-all"
                        />
                    </div>
                    {!loading && !error && entradas.length > 0 && (
                        <div className="flex flex-wrap items-center gap-4">
                            ...
                            <span className="text-xs font-bold text-slate-500 dark:text-slate-400">
                                Total recebido: <span className="text-slate-900 dark:text-slate-100">{totalUnidades} unidades</span>
                            </span>
                            <div className="w-px h-4 bg-slate-200 dark:bg-slate-700"></div>
                            <span className="text-xs font-bold text-slate-500 dark:text-slate-400">
                ...
                <div className="flex-1 overflow-y-auto p-6 bg-slate-50 dark:bg-slate-900 custom-scrollbar">
                    {loading ? (
                        <div className="flex flex-col items-center justify-center py-20 gap-3">
                            ...
                            <p className="text-sm text-slate-500 dark:text-slate-400 font-medium">A carregar histórico...</p>
                        </div>
                    ) : error ? (
                        <div className="flex items-center gap-3 bg-red-50 dark:bg-red-500/10 border border-red-100 dark:border-red-500/20 text-red-700 px-5 py-4 rounded-xl">
                        ...
                        <div className="bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm overflow-hidden">
                            <table className="w-full text-left text-xs">
                                <thead className="bg-slate-50 dark:bg-slate-900/80 border-b border-slate-200 dark:border-slate-700 sticky top-0">
                                    <tr>
                                        <th className="px-4 py-3 font-black text-slate-500 dark:text-slate-400 uppercase tracking-wider">Produto</th>
                                        <th className="px-4 py-3 font-black text-slate-500 dark:text-slate-400 uppercase tracking-wider">Encomenda</th>
                                        <th className="px-4 py-3 font-black text-slate-500 dark:text-slate-400 uppercase tracking-wider">Fornecedor</th>
                                        <th className="px-4 py-3 font-black text-slate-500 dark:text-slate-400 uppercase tracking-wider text-center">Qtd. Recebida</th>
                                        <th className="px-4 py-3 font-black text-slate-500 dark:text-slate-400 uppercase tracking-wider text-right">Valor</th>
                                        <th className="px-4 py-3 font-black text-slate-500 dark:text-slate-400 uppercase tracking-wider">Data</th>
                                    </tr>
                                </thead>
                                <tbody className="divide-y divide-slate-50">
                                    {filtered.map((entrada) => {
                                        ...
                                        return (
                                            <tr key={entrada.id} className="hover:bg-slate-50 dark:hover:bg-slate-700/50 dark:bg-slate-900 transition-colors">
                                                <td className="px-4 py-3.5">
                                                    <div className="flex items-center gap-2.5">
                                                        <div className="w-7 h-7 rounded-lg bg-emerald-50 dark:bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 flex items-center justify-center shrink-0 border border-emerald-100 dark:border-emerald-500/20">
                                                            <Package size={13} />
                                                        </div>
                                                        <div>
                                                            <p className="font-bold text-slate-900 dark:text-slate-100">{entrada.produto.nome}</p>
                                                ...
                                                <td className="px-4 py-3.5">
                                                    <div>
                                                        ...
                                                        <span className={`inline-flex items-center gap-1 mt-1 px-1.5 py-0.5 rounded text-[9px] font-black uppercase tracking-wider ${
                                                            isCompleta ? 'bg-emerald-50 dark:bg-emerald-500/10 text-emerald-700 border border-emerald-100 dark:border-emerald-500/20' :
                                                            isParcial ? 'bg-orange-50 text-orange-700 border border-orange-100' :
                                                            'bg-blue-50 dark:bg-blue-500/10 text-blue-700 border border-blue-100 dark:border-blue-500/20'
                                                        }`}>
                                                ...
                                                <td className="px-4 py-3.5 text-center">
                                                    <span className="inline-flex items-center justify-center px-2.5 py-1 rounded-lg bg-emerald-50 dark:bg-emerald-500/10 text-emerald-700 font-black border border-emerald-100 dark:border-emerald-500/20 text-xs">
                ...
                <div className="px-6 py-4 bg-slate-50 dark:bg-slate-900 border-t border-slate-100 dark:border-slate-700/50 flex items-center justify-between">
                    ...
                    <button
                        onClick={onClose}
                        className="px-5 py-2 text-sm font-bold text-slate-600 dark:text-slate-400 hover:text-slate-800 dark:text-slate-200 hover:bg-slate-200 dark:bg-slate-700 rounded-xl transition-all"
                    >
                        Fechar
                    </button>
                </div>
